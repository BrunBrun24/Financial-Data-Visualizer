...
class OperationCategoriser:
    ...
    Attributs:
        ...
        - `tableaux` (dict): Dictionnaire de DataFrames, chaque clé étant une valeur unique de 'LIBELLÉ COURT'.
        - `results` (dict): Dictionnaire pour stocker les opérations catégorisées.
        - `history` (list): Liste des opérations traitées avec leurs catégories.
        - `listesTypesOperation` (dict): Dictionnaire de listes d'opérations regroupées par catégorie.
        - `root` (tk.Tk): Instance de la fenêtre Tkinter.
        - `windowWidth` (int): Largeur de la fenêtre Tkinter.
        - `windowHeight` (int): Hauteur de la fenêtre Tkinter.
        - `index` (int): Index de l'opération courante.
        - `categoryIndex` (int): Index de la catégorie courante.
        - `displayLabel` (tk.Label): Étiquette Tkinter pour afficher des informations.

    Méthodes:
        - `__init__(self, data, buttonLabels, cheminFileJson, buttonsPerRow=5)`: Initialise les attributs de la classe.
        - `TrierDonnees(donnees)`: Trie les données en créant des DataFrames distincts pour chaque valeur unique de 'LIBELLÉ COURT'.
        - `CenterWindow(self)`: Centre la fenêtre Tkinter sur l'écran.
        - `ReorderKeys(item, index)`: Réorganise les clés d'un dictionnaire pour que la clé 'Type' soit à la position spécifiée.
        - `TrierElements(self, dictionnaire)`: Réorganise les éléments du dictionnaire pour que la clé 'Type' soit en deuxième position.
        - `ComparerEtCategoriserOperations(self, operationsActuelles)`: Compare les opérations actuelles avec celles existantes dans le fichier JSON et les catégorise.
        - `AfficherFenetreAvecBoutons(self)`: Affiche une fenêtre Tkinter avec des boutons pour catégoriser les opérations.
        - `UpdateDisplay(self)`: Met à jour l'affichage en fonction des opérations à classer et des boutons à créer.
        - `CreateButtons(self, filteredButtons)`: Crée des boutons pour les catégories ou sous-catégories filtrées et ajoute des boutons de navigation.
        - `ButtonClicked(self, buttonName)`: Gère le clic sur un bouton représentant une catégorie principale 
          ou une sous-catégorie, en mettant à jour l'affichage en conséquence.
        - `SubButtonClicked(self, sub_button_name, parent_button_name)`: Gère le clic sur un bouton de sous-catégorie 
          en traitant la sous-catégorie et la catégorie principale associée.
        - `GoBack(self)`: Permet de revenir à l'état précédent en utilisant l'historique des actions.
        - `SkipEntry(self)`: Passe à l'entrée suivante dans la liste des opérations à catégoriser.
        - `ProcessButton(self, buttonName)`: Traite le clic sur un bouton représentant une catégorie ou une sous-catégorie 
          en attribuant l'entrée courante à la catégorie sélectionnée.
        - `ProcessSubButton(self, sub_button_name, parent_button_name)`: Traite le clic sur un bouton de sous-catégorie 
          en attribuant l'entrée courante à la sous-catégorie sélectionnée.
        - `ProcessSpecialCases(self, currentItem, currentList)`: Traite les cas spéciaux pour les opérations et 
          les catégorise en conséquence.
        - `PutInCategorie(self, currentItem, currentList, mainCategorie, secondCategorie)`: Attribue une entrée à 
          une catégorie principale et une sous-catégorie spécifiées, puis met à jour les résultats et l'historique.
    """
